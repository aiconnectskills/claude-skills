...
# [SECURITY] Allowed hosts for pagination URLs. Meta embeds the access
# token into the `paging.next` URL, so following a redirect to a non-Meta
# host would leak the token.
ALLOWED_PAGINATION_HOSTS = {"graph.facebook.com"}
...
def _request(
    ...
    *,
    ...
    for attempt in range(max_retries):
        try:
            resp = requests.request(method, url, params=params, data=data, timeout=60, verify=True)
...
def _is_safe_pagination_url(url: str) -> bool:
    """Validate that a pagination URL points to graph.facebook.com.

    Meta embeds the access token into paging.next URLs. A compromised or
    spoofed API response could include a next URL pointing to a malicious
    host, which would leak the token. This check prevents that.
    """
    try:
        parsed = urllib.parse.urlparse(url)
        return (
            parsed.scheme == "https"
            and parsed.hostname in ALLOWED_PAGINATION_HOSTS
        )
    except Exception:
        return False
...
def paginate(path: str, params: dict | None = None, max_pages: int = 100) -> Iterator[dict]:
    ...
    while page < max_pages:
        if next_url:
            # [SECURITY] Validate the URL before following it — Meta embeds
            # the token in the URL, so we must ensure it stays on Meta's servers.
            if not _is_safe_pagination_url(next_url):
                print(
                    f"[meta_client] Refusing to follow pagination URL to untrusted host: "
                    f"{urllib.parse.urlparse(next_url).hostname}",
                    file=sys.stderr,
                )
                break

            try:
                resp = requests.get(next_url, timeout=60, verify=True)
