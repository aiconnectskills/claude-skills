...
1. **Pick the right script.** Don't reinvent. The bundled scripts cover the common cases:
   ...
   - `update_budget.py` — modify daily or lifetime budget with 2x-per-call safety cap
...
1. **Surface the action and the impact in plain language before calling.** Example: "Pause ad `Campaign_v3` (ID 120214...). This ad spent 47 ILS in the last 7 days with 0.8% CTR. Confirm?"
...
Per `write-actions.md`, a fresh explicit confirmation is required for `--confirm` even if the user approved the plan earlier in the same session. That confirmation must name the action (e.g. "confirm create") — not a generic "ok".
